#include "ThingspeakConnection.h"
  ...
  bool ThingspeakConnection::tryPushToThingSpeak(float bat, int sensor1, int sensor2) {
  ...
  /// different one.
  bool ThingspeakConnection::sendCommand(char* command, char* expectedAnswer,
  ...
  void ThingspeakConnection::modemSerialBridgeLoop() {
    char incoming_char;
  ...
  bool ThingspeakConnection::waitFor(char* answer, unsigned int timeout) {
    Timeout t(timeout);
    while (*answer != '\0') {
  ...
  /// otherwise true.
  bool ThingspeakConnection::waitForTCPStack(unsigned int timeout) {
    Timeout t(timeout);
    while (
  ...
  bool ThingspeakConnection::readUntil(char* answer, unsigned int timeout) {
  ...
  /// timeout elapsed.
  bool ThingspeakConnection::readUntil(char c, unsigned int timeout) {
    char inchar = c - 1;
    Timeout t(timeout);
    while (inchar != c) {
  ...
  bool ThingspeakConnection::waitForNextChar(unsigned int timeout) {
    Timeout t(timeout);
